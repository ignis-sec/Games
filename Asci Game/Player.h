...
public:
	...
	Player(int x, int y, wchar_t tag, int delay, Direction Direction, WORD Attribute) : Actor(x, y, tag, delay, Direction, Attribute) { g_HP = getHP(); ThisPlayer = this; } //inherits constructor from Actor base class
	void ActorTick() {
		...
		if (getLife() % 2)
		{
			if (bKeyUp)		AddPosition(0, -1);
			if (bKeyDown)	AddPosition(0, 1);
		}
		if (bKeyRight)	AddPosition(1, 0);
		if (bKeyLeft)	AddPosition(-1, 0);
	...
	void OnCollision(Collision C) {
		...
		{
			if (C.Instigator->GetTag() == L'S') { decreaseHP(); }
		}
